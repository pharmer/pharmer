...
var KubeProviders = &kubeProviderExt{
	newExtensionPoint(new(KubeProvider)),
}

type kubeProviderExt struct {
	*extensionPoint
}

func (ep *kubeProviderExt) Unregister(name string) bool {
	return ep.unregister(name)
}

func (ep *kubeProviderExt) Register(extension KubeProvider, name string) bool {
	return ep.register(extension, name)
}

func (ep *kubeProviderExt) Lookup(name string) KubeProvider {
	ext := ep.lookup(name)
	if ext == nil {
		return nil
	}
	return ext.(KubeProvider)
}

func (ep *kubeProviderExt) Select(names []string) []KubeProvider {
	var selected []KubeProvider
...
func (ep *kubeProviderExt) All() map[string]KubeProvider {
	all := make(map[string]KubeProvider)
	for k, v := range ep.all() {
		all[k] = v.(KubeProvider)
	}
	return all
}

func (ep *kubeProviderExt) Names() []string {
	var names []string
	for k := range ep.all() {
