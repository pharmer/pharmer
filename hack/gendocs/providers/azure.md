...
NAME                    AGE
{{ .Provider.ClusterName }}-master-0        27m
{{ .Provider.ClusterName }}-master-1        27m
{{ .Provider.ClusterName }}-master-2        27m
{{ .MachinesetName }}-smnwg 27m
      ...
      providerSpec:
        value:
          ...
          networkSpec:
            vnet:
              name: ""
          resourceGroup: {{ .Provider.ClusterName }}
          saKeyPair:
            cert: null
            key: null
          sshPrivateKey: ""
  ...
  config:
    ...
    cloud:
      azure:
        ...
        azureStorageAccountName: k8s{{ .Provider.ClusterName }}by5v4x
...
spec:
  clusterApi:
        ...
        value:
          ...
          adminKubeconfig: |
            ...
            - cluster:
                ...
                server: https://{{ .Provider.ClusterName }}-d30d08c.eastus2.cloudapp.azure.com:6443
              name: {{ .Provider.ClusterName }}.pharmer
            contexts:
            - context:
                cluster: {{ .Provider.ClusterName }}.pharmer
                user: cluster-admin@{{ .Provider.ClusterName }}.pharmer
              name: cluster-admin@{{ .Provider.ClusterName }}.pharmer
            current-context: cluster-admin@{{ .Provider.ClusterName }}.pharmer
            kind: Config
            preferences:
              colors: true
            users:
    ...
    status:
      apiEndpoints:
      - host: {{ .Provider.ClusterName }}-d30d08c.eastus2.cloudapp.azure.com
        port: 6443
      providerStatus:
        bastion:
          image:
        ...
        network:
          apiServerIp:
            dnsName: {{ .Provider.ClusterName }}-d30d08c.eastus2.cloudapp.azure.com
            name: {{ .Provider.ClusterName }}-d30d08c
          apiServerLb:
            backendPool: {}
            frontendIpConfig: {}
          bastionIP:
            dnsName: {{ .Provider.ClusterName }}-bastion-d30d08c.eastus2.cloudapp.azure.com
            name: {{ .Provider.ClusterName }}-bastion-d30d08c
  config:
    masterCount: 3
    cloud:
      cloudProvider: azure
...
status:
  ...
  cloud:
    loadBalancer:
      dns: {{ .Provider.ClusterName }}-d30d08c.eastus2.cloudapp.azure.com
      ip: 52.232.224.99
      port: 6443
```
{{ end }}
...
{{ .Provider.ClusterName }}-master-0               Ready    master   24m     {{ .KubernetesVersion }}
{{ .Provider.ClusterName }}-master-1               Ready    master   11m     {{ .KubernetesVersion }}
{{ .Provider.ClusterName }}-master-2               Ready    master   17m     {{ .KubernetesVersion }}
standard-b2ms-pool-fpvhk   Ready    node     7m23s   {{ .KubernetesVersion }}
```
{{ end }}
...
```console
capi@{{ .Provider.ClusterName }}-bastion:~$ ssh 10.0.0.4
capi@{{ .Provider.ClusterName }}-master-0:~$
```
{{ end }}
