...
func newCmdIGCreate() *cobra.Command {
	cluster := &api.Cluster{}
	nodes := map[string]int{}
	cmd := &cobra.Command{
		Use:               "ng",
		Short:             "Create a Kubernetes cluster node group for a given cloud provider",
		Example:           "ng --nodes=t1=1,t2=2  demo-cluster",
		DisableAutoGenTag: true,
		Run: func(cmd *cobra.Command, args []string) {
