...
func (cm *clusterManager) create(req *proto.ClusterCreateRequest) error {
	...
	cm.ctx.Notifier.StoreAndNotify(api.JobStatus_Running, fmt.Sprintf("Linode kernel %v found", cm.ctx.Kernel))
	...
	if api.UseFirebase() {
		common.SaveInstancesInFirebase(cm.ctx.NewScriptOptions(), cm.ins)
	}
		...
		for _, info := range nodes {
			if info.state == 0 {
				...
				if err == nil {
					...
					cm.ctx.Notifier.StoreAndNotify(api.JobStatus_Running, fmt.Sprintf("Instance %v (%v) is %v", linode.Label, linode.LinodeId, statusString(linode.Status)))
