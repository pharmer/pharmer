...
func (cm *ClusterManager) Delete(req *proto.ClusterDeleteRequest) error {
	...
	for _, i := range instances {
		...
		Logger(cm.ctx).Infof("Linode %v with id %v for clutser is deleted", i.Name, i.Status.ExternalID, cm.cluster.Name)
	}

	backoff.Retry(cm.deleteStackscripts, backoff.NewExponentialBackOff())
	Logger(cm.ctx).Infof("Stack scripts for cluster %v deleted", cm.cluster.Name)
