...
func (cm *ClusterManager) deleteResourceGroup(groupName string) error {
	...
	Logger(cm.ctx).Infof("Resource group %v deleted", groupName)
	return <-errchan
}
...
func (cm *ClusterManager) deleteNodeNetworkInterface(interfaceName string) error {
	...
	Logger(cm.ctx).Infof("Node network interface %v deleted", interfaceName)
	return <-errchan
}
...
func (cm *ClusterManager) deletePublicIp(ipName string) error {
	...
	Logger(cm.ctx).Infof("Public ip %v deleted", ipName)
	return <-errchan
}
func (cm *ClusterManager) deleteVirtualMachine(machineName string) error {
	...
	Logger(cm.ctx).Infof("Virtual machine %v deleted", machineName)
	return <-errchan
}
