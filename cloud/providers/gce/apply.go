...
func (cm *ClusterManager) Apply(in *api.Cluster, dryRun bool) ([]api.Action, error) {
	...
	if cm.cluster.Status.Phase == api.ClusterReady {
		...
		if upgrade, err := NewKubeVersionGetter(kc, cm.cluster).IsUpgradeRequested(); err != nil {
			...
			_, _ = Store(cm.ctx).Clusters().UpdateStatus(cm.cluster)
	...
	if cm.cluster.DeletionTimestamp != nil && cm.cluster.Status.Phase != api.ClusterDeleted {
		machines, err := Store(cm.ctx).MachineSet(cm.cluster.Name).List(metav1.ListOptions{})
		if err != nil {
			return nil, err
		}
		...
		for _, machine := range machines {
			...
			_, err := Store(cm.ctx).MachineSet(cm.cluster.Name).Update(machine)
			if err != nil {
				return nil, err
			}
		}
...
func (cm *ClusterManager) applyCreate(dryRun bool) (acts []api.Action, err error) {
	...
	if providerSpec.MachineType == "" {
		...
		for _, m := range machines {
			...
			_, err = Store(cm.ctx).Machine(cm.cluster.Name).Update(m)
			if err != nil {
	...
	leaderMachine, err = Store(cm.ctx).Machine(cm.cluster.Name).UpdateStatus(leaderMachine)
	if err != nil {
		return
	}
	...
	for _, m := range machines {
		...
		if _, err := client.ClusterV1alpha1().Machines(cm.cluster.Spec.ClusterAPI.Namespace).Create(m); err != nil && !errAlreadyExists(err) && !api.ErrObjectModified(err) {
	...
	for _, machine := range machines {
		if !util.IsControlPlaneMachine(machine) {
			...
			if !dryRun {
				...
				err = Store(cm.ctx).Machine(cm.cluster.Name).Delete(machine.Spec.Name)
				if err != nil {
					return nil, err
				}
				err = Store(cm.ctx).MachineSet(cm.cluster.Name).Delete(machine.Name)
				if err != nil {
					return nil, err
				}
			}
