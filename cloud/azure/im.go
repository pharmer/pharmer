...
func (im *instanceManager) createPublicIP(name string, alloc network.IPAllocationMethod) (network.PublicIPAddress, error) {
	...
	im.ctx.Notifier.StoreAndNotify(api.JobStatus_Running, fmt.Sprintf("Public ip addres %v created", name))
...
func (im *instanceManager) createNetworkInterface(name string, subnet network.Subnet, alloc network.IPAllocationMethod, internalIP string, pip network.PublicIPAddress) (network.Interface, error) {
	...
	im.ctx.Notifier.StoreAndNotify(api.JobStatus_Running, fmt.Sprintf("Network interface %v created", name))
...
func (im *instanceManager) createVirtualMachine(nic network.Interface, as compute.AvailabilitySet, sa armstorage.Account, vmName, data, vmSize string) (compute.VirtualMachine, error) {
	...
	im.ctx.Notifier.StoreAndNotify(api.JobStatus_Running, fmt.Sprintf("Virtual machine with disk %v password %v created", im.namer.BootDiskURI(sa, vmName), im.ctx.InstanceRootPassword))
	...
	im.ctx.Notifier.StoreAndNotify(api.JobStatus_Running, fmt.Sprintf("Restarting virtual machine %v", vmName))
	...
	im.ctx.Notifier.StoreAndNotify(api.JobStatus_Running, fmt.Sprintf("Found virtual machine %v", vm))
	return vm, err
}
...
func (im *instanceManager) DeleteVirtualMachine(vmName string) error {
	...
	im.ctx.Notifier.StoreAndNotify(api.JobStatus_Running, fmt.Sprintf("Virtual machine %v deleted", vmName))
